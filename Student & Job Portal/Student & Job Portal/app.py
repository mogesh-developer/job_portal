...
def inject_user_context():
    ...
    if session.get('username'):
        ...
        try:
            ...
            user_image = user_row.get('profile_image') if user_row else None
...
def debug_avatar():
    ...
    img = session.get('user_image')  # this won't exist; instead fetch via context_processor logic
    return render_template_string("<p>Username: {{session.username}}</p><p>User image (from context): {{ user_image }}</p>"
                                  "<img src='{{ url_for('static', filename=user_image) }}' style='max-width:200px;' onerror='this.onerror=null;this.src=\"{{ url_for('static', filename=\"defaults/avatar.png\") }}\";'>",
